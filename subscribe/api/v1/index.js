...
lsq.config.get().then(function(c){
  config = c
  console.log(config.api.mailchimp.token)
...
api.post('/demo/subscribe',function(req,res){
  ...
  mc.lists.subscribe({ id: config.api.mailchimp.lists.lsq, email: { email:req.body.email }, merge_vars: { SOURCE: req.body.source, TEAMSIZE: req.body.teamsize|| ""}, double_optin:false, send_welcome:false }, 
    function(data) {
